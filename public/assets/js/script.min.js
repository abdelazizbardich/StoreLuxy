$(document).ready(function () {
 ...
 $(".qte_input .plus").click(function () {
  ...
  if(maxQte == value){Notiflix.Notify.Warning('ليس لدينا المزيد في المخزون'); return false;}
  if (!value) { value = 0; }
  value = parseInt(value);
  $(this).parents(".qte_input").find('input').val(value + 1);
 });
 ...
 $(".btn-add-to-carte").click(function (e) {
  ...
  $.get({
   ...
   success: function (res) {
     if (res == '0') {
        Notiflix.Notify.Warning('المنتج موجود بالفعل في سلة التسوق');
     ...
     Notiflix.Notify.Success('تمت إضافة المنتج إلى سلة التسوق');
     $("#fixed-carte").css('transform', 'translateX(0px)');
 ...
 $('#newsletterForm').submit(function (e) {
  ...
  $.post({
   ...
   success: function (res) {
     console.log(res);
     if (res == 1) {
        Notiflix.Notify.Success('شكرا لك على الاشتراك في النشرة الإخبارية');
 ...
 $.get({
  ...
  success: function (res) {
   if (res == 1) {
     let cartCountBadge = $('.cart-count-badge').text();
     $('.cart-count-badge').text(parseInt(cartCountBadge) - 1);
     Notiflix.Notify.Success('تمت إزالة المنتج من عربة التسوق');
   }
  }
 });
